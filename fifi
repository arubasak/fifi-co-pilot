...
try:
    ...
    st.error(
        f"Missing critical secret: {e}. Please ensure this secret is configured in your Streamlit Cloud app settings or local secrets.toml.")
...
def initialize_pinecone_assistant():
    ...
    try:
        ...
        assistant = pc.assistant.Assistant(
            assistant_name=ASSISTANT_NAME)  # Assuming region is handled by client or not needed here
        return assistant
    except PineconeApiException as e:
        st.error(
            f"Pinecone API Error during initialization: {e}. Ensure '{ASSISTANT_NAME}' exists and API key is valid.")
        return None
...
def handle_user_query(user_query: str):
    ...
    messages_for_sdk_conversion = [{"role": msg["role"], "content": msg["content"]} for msg in
                                   st.session_state.messages]
    ...
    if conversion_error_message:
        ...
        try:
            with st.spinner("FiFi is thinking..."):
                response_from_sdk = assistant.chat(messages=pinecone_messages_for_sdk,
